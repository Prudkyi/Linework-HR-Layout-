(function() {
	...
	var app = {
		...
		forms: function() {
			...
			$(".сallback_popup_form").validate({
				...
				messages: {
					Name: {
						required: "Поле 'Имя' не может быть пустое.",
					},
					Phone: {
						required: "Поле 'Телефон' не может быть пустое.",
					}
				},
	...
	let messButStatus = 'closed',
	    messBut = $('.messChat_but_el'),
	    messButIcon = $('.messChat_but_el i'),
		messList = $('.messChat_list');


	function messOpen() {
		messList.css('display', 'flex');
		messList.show();
		messButStatus = 'open';
		messButIcon.html('closed');
	}

	function messClosed() {
		messList.hide();
		messButStatus = 'closed';
		messButIcon.html('forum');
	}

	messBut.on('click', function () {
		if (messButStatus == 'closed')
		{
			messOpen()
		}
		else
